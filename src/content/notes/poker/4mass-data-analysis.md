---
title: "Intro to MDA"
tags: ["poker"]
date: "12/26/25"
category: "Poker"
---

##### What is Mass Data Analysis (MDA)?
MDA is a gigantic database of hands processed to show us the most common and frequent leaks in humans playing poker. Most people make the same mistakes, normally distributed.  
Two distinctions: Regs and Fish. Regs want to play good poker and fish are here to gamble and not trying to be better.  
MDA looks at overfolded and underfolded spots as well as overbluffed and underbluffed spots in the game.  
Use MDA as a strong suggestion, know when to follow this data and make connections with theory. When to follow/deviate. Many spots where MDA and GTO agree, and there exists spots where they disagree. For instance, 3BP B-B-B line is overfolded and theory shows that because on a Q5272r runout, facing an all in, the player needs to find calls with 33, 54, 66, 77, 88, 99 which is hard to find. Thus, theory backs up the overfolded texture of this line.  
Studying MDA not only helps you exploit villians, but can also help you self examine your own leaks and flawed playstyle.

##### Bluff Catching Regs

<details>
  <summary>Overall Summary</summary>
    
  <div style="margin-left: 1.5em">
    5 things to look for when determining whether villian is bluffing.


- **Position**
    - later positions significantly overbluffed (BTN-BB)
    - CO is neutral
    - EP-MP underbluffed


- **Board Texture**
    - lower disconnected boards overbluffed
    - 4 Flush River significantly overbluffed in all discontinued lines and underbluffed in continued lines
    - triple broadway, 4 flush turn, monotone, and A high boards are underbluffed
    - everything else neutral

    
- **Line**
    - discontinued overbluffed
    - continued underbluffed


- **Flop Bet Size**
    - larger less likely bluffing
    - 0-33% overbluffed (including a X)
    - 75%+ underbluffed


- **River Bet Size**
    - 50% and lower is overbluffed
    - 90%+ is underbluffed


At the end of the day, either you opponent is bluffing enough and overbluffing and you call everything or they are underbluffing and you are folding bluffcatchers. Even a slight underbluff removes all bluffcatchers on the river. 
  </div>

</details>

<details>
This is from the villian's POV (villian is SRP IP PFR so we are SRP OOP PFC)
  <summary>SRP IP PFR</summary>
    
  <div style="margin-left: 1.5em">
    <details>
        <summary>B-B-B underbluffed</summary>  
        - monotone boards and low boards underbluffed
    </details>
    <details><summary>B-X-B overbluffed</summary></details>
    <details><summary>X-B-B overbluffed</summary></details>
    <details><summary>X-X-B extremely overbluffed</summary></details>
  </div>

</details>
<details>
  <summary>SRP OOP PFC</summary>

  <div style="margin-left: 1.5em">
    <details><summary>XR-B-B extremely underbluffed</summary></details>
    <details><summary>XR-X-B overbluffed</summary></details>
    <details><summary>XC-X-B overbluffed</summary></details>
    <details><summary>X-X-B extremely overbluffed</summary></details>
  </div>

</details>
<details>
  <summary>3! IP PFR</summary>

  <div style="margin-left: 1.5em">
    <details><summary></summary></details>
  </div>

</details>
<details>
  <summary>3! OOP PFC</summary>

  <div style="margin-left: 1.5em">
    <details><summary></summary></details>
  </div>

</details>
<details>
  <summary>3! OOP PFR</summary>

  <div style="margin-left: 1.5em">
    <details><summary></summary></details>
  </div>

</details>
<details>
  <summary>4! IP PFR</summary>

  <div style="margin-left: 1.5em">
    <details><summary></summary></details>
  </div>

</details>
<details>
  <summary>4! OOP PFR</summary>

  <div style="margin-left: 1.5em">
    <details><summary></summary></details>
  </div>

</details>

##### Reg Folding Frequency
Overall, regs overfold across the board. Hard to make a -EV river bluff vs a reg that has no read on you because they overfold on all sizings. They overfold the most when they cap their range by checking or checking back a street and overfold the least when they take lines with continued agression.

##### Bluff Raising on the River VS Reg
There are three main factors when bluff raising regs:
1. Villian being OOP
2. Villian taking discountinued line
3. Villian river betsize (the smaller the better)  
In general, the more these factors are met, the more overfolded the situation is. You can start bluffing random trast the more these factors are met. 

##### Flop and Turn Overfolds from Reg
SRP IP PFR: X-F line is overfolded
SRP OOP PFC: XR-XF is extremely overfolded., X-XF is overfolded.
##### Exploiting Fish with MDA
To be continued...
##### Fish folding frequency
To be continued...